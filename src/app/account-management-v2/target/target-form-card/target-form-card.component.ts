...
@Component({
  selector: 'app-target2-form-card',
...
export class TargetFormCardComponent implements OnInit, OnChanges {
  ...
  constructor(private _metadataService: MetadataService, @Inject(MAT_DIALOG_DATA) public modelData: any) { }
  ...
  private createFields() {
    const companyDropDown = new SelectOption('companyName', 'companyStringId', this._metadataService.companyDropdownList);
    ...
    const managerDropDown = new SelectOption('salesManagerName', 'salesManagerId', this._metadataService.salesManagerList);
